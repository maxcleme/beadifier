...
export class XlsxPrinter implements Printer {
  ...
  pattern(workbook, reducedColor: Uint8ClampedArray, project: Project) {
    ...
    for (let y = 0; y < height; y++) {
      ...
      for (let x = 0; x < width; x++) {
        ...
        if (paletteEntry) {
          const fg = `FF${ColorToHex(foreground(paletteEntry.color)).substring(
            1
          )}`;
          const bg = `FF${ColorToHex(paletteEntry.color).substring(1)}`;
  ...
  usage(workbook, usage: Map<string, number>, project: Project) {
    ...
    let y = 0;
    let refIdx = 1;
    let symbolIdx = project.exportConfiguration.useSymbols ? 2 : -1;
    let countIdx = project.exportConfiguration.useSymbols ? 3 : 2;
    ...
    Array.from(usage.entries())
      ...
      .forEach(([k, v]) => {
        ...
        const fg = `FF${ColorToHex(foreground(entry.color)).substring(1)}`;
